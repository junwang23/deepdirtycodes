...
cell_stack = [tf.contrib.rnn.BasicLSTMCell(state_size, state_is_tuple=True) for i in range(num_layers)]
cells = tf.contrib.rnn.MultiRNNCell(cell_stack, state_is_tuple=True)
states_series, current_state = tf.contrib.rnn.static_rnn(cells, inputs_series, initial_state=rnn_tuple_state)
